...
def parsing():
    ...
    parser.add_argument(
        "-w", "--wigfiles", help="write intermediate wig files", action="store_true"
    )
    parser.add_argument("-o", metavar="OUTDIR", help="output directory", default=".")
    parser.add_argument(
        "-g", metavar="GENOMEFILE", help="FASTA genome file", required=True
    )
    parser.add_argument(
        "-a", metavar="ALIGNFILE", help="BOWTIE alignment file", required=True
    )
    parser.add_argument(
        "-p", metavar="PREFIXNAME", help="Prefix name for result files", required=True
    )
...
def buscapicos(signals, umbral, vecindad):
    ...
    for i in range(1, len(picos_ord) - 1):
        if (
            picos_ord[i] - picos_ord[i - 1] > vecindad
            and picos_ord[i + 1] - picos_ord[i] > vecindad
        ):
            coord = picos_ord[i]
...
def denoising(signal):
    ...
    for i in range(1, level + 1):
        coeffs[i] = np.array([0] * len(coeffs[i]))
...
if wig_int:
    ...
    for chrid in chrseq.keys():
        WIG.write(
            'track type=wiggle_0 name=%s_%s description="%s_%s"\n'
